...
const int MARGIN_BOTTOM = 30; // Margen desde la parte inferior de la imagen
...
// Tamaño de fuente para el texto principal de "Escucha sin Subtítulos"
const int FONT_HEIGHT_MAIN_TEXT = 120; // Ajustado a casi el doble para mayor impacto

// Grosor del contorno (outline) del texto
const int OUTLINE_THICKNESS = 5; // Ajusta este valor para un contorno más grueso o fino
...
int calculateWrappedTextHeight(Ptr<freetype::FreeType2> ft2, const string& text, int fontHeight, int maxWidth) {
    ...
    // En este contexto, no se añade espaciado extra entre líneas para mantenerlo compacto.
...
void drawWrappedTextWithOutline(
    ...
    for (const string& line : lines) {
        ...
        // Dibuja el contorno
        // Se itera alrededor del punto central para crear el "stroke"
        for (int i = -outlineThickness; i <= outlineThickness; ++i) {
            for (int j = -outlineThickness; j <= outlineThickness; ++j) {
                if (i == 0 && j == 0) continue; // Omite el centro, donde irá el texto principal
...
int main() {
    ...
    int rect_y = IMG_HEIGHT - rect_height - MARGIN_BOTTOM; // Basado en el margen inferior
    ...
    // Usar el nuevo color celeste azulado para el rectángulo
    ...
    string output_filepath = output_dir + "/prueba_output.png";
